...
(function() {
    ...
    function createManagementButton() {
        const button = document.createElement('button');
        button.id = 'channel-blocker-btn';
        button.textContent = '🛡️ 频道管理 v23';
        button.style.cssText = `
            position: fixed;
            bottom: 20px;
            right: 20px;
    ...
    function updateButtonVisibility() {
        const button = document.getElementById('channel-blocker-btn');
        if (!button) return;
        
        const currentUrl = window.location.href;
        // 只在首页、搜索页、频道页等显示，不在视频播放页显示
    ...
    function init() {
        ...
        const observer = new MutationObserver(() => {
            hideBlockedChannels();
            addBlockButtons();
            updateButtonVisibility(); // 监听 URL 变化
        });
